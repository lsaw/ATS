	...
	if($_POST['sysLinkChanged']== 'true'){
		...
		if($num){ // this syslink exists so modify
			$query = 'UPDATE systemlinks SET ' . 
						 ...
						 pmis_year = ?';
			$paramTypes = 'sssisi';
			$params = array(	$_POST['pmisNum'] ,
								$_POST['pmisPart'],
								$_POST['pmisName'],
								$dol,
								$_POST['pmisFundName'],
								$_POST['pmisYear']);
			if(!('' == $_POST['pmisRegRank'])){// there is some rank, so include it in update
				$query .=',
						pmis_region_rank = ?';
				$paramTypes .= 'i';
				$params[] = $_POST['pmisRegRank'];
			}
			// if no rank, leave off so sql will use default null value
			// then finish query with WHERE statement
			...
			$query = "INSERT INTO systemlinks (" .
								"asset_id, pmis_number, pmis_part, pmis_name, pmis_dollars, pmis_fund_name, pmis_year";
			$values = " VALUES (?,?,?,?,?,?,?";
			$paramTypes = 'sssssss';
			$params = array(	$assID,
								$_POST['pmisNum'] ,
...
if(isset($_POST['projNum']))// this is the project number passed in from manage assets
	$projectNum = $_POST['projNum'];
...
echo "		<div id='container'>
			...
			<td><a href='tier.php' onclick='return confirmMove();'>Tier Assignment</a></td>
			<td><a href='managePMIS.php?park=$park&projNum=$projectNum' onclick='return confirmMove();'>Manage PMIS</a></td>
			<td><a href='manageAssets.php?park=$park&projNum=$projectNum' onclick='return confirmMove();'>Manage Assets</a></td>
			<td><a href=\"FMSS.php?park=$park&asset_id=$asset_id&asset_name=$asset_name&projNum=$projectNum\" onclick='return confirmMove();'>FMSS Links</a></td>
			<td><a href=\"funding.php?park=$park&asset_id=$asset_id&asset_name=$asset_name&projNum=$projectNum\" onclick='return confirmMove();'>Funding</a></td>
...
echo"						<tr>	<td></td>
						...
						<tr><td><input type='hidden' id='idAssetID' name='assetId' value='$asset_id'>
								<input type='hidden' id='idParkName' name='park' value='$park'>
								<input type='hidden' id='idProjNum' name='projNum' value='$projectNum'>
...
//echo "						<tr>	<td><select id='idProjectNum$i' name='projectNumber$i' style='width:100%;' onChange=editFunction(this,'projC')>
//";
