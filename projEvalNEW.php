...
$projectNum = isset($_POST['projNum'])		? $_POST['projNum'] 	: '';
...
$projectNum = isset($_GET['projNum'])		? $_GET['projNum'] 		: $projectNum;
...
//$park = '';
//$projectNum = '';
//$yearPicked = $yearSet = $miles = $ridersPerV = $annualRiders = '';
//if(isset($_POST['parkName']))
//	$park = $_POST['parkName'];
//if(isset($_POST['projectNum']))
//	$projectNum = $_POST['projectNum'];
//if(isset($_POST['yearPicked']))
//	$yearPicked = $_POST['yearPicked'];
//if(isset($_POST['yearSet']))
//	$yearSet = $_POST['yearSet'];
//$yearSet = $yearSet=='test' ? 9999 : $yearSet;
...
if(isset($_POST['postResult']) && $yearSet !=''){
	$yearPicked = $yearSet;
...
// need to deal with situation where you have a project selected for park A, but you
// then switch to park B, and that park does not have that project.  Sol'n is to
// query that park for that project. If not there force to ''.

// Get list of region names form database and load into select field
$resRegions = getRegions($mysqli);

// Get list of park names from database and load into select field
//$resParks = getSingleColumnQueryMysqli($mysqli, 'park_id', 'parks');
$resParks = getParks($mysqli, $region);

//if(!isset($_POST['park']) && !isset($_POST['project']) && !isset($_POST['yearP']) && !isset($_POST['postRes'])){
///	$resProj = getProjects($mysqli, '');
//}

// Get list of project names from projects database and load into select field
//if(isset($_POST['park'])){
//	$resProj = getProjects($mysqli, $park);
//	$yearPicked = '';
//}

//if(isset($_POST['yearPicked']) || isset($_POST['postRes'])){
	$resProj = getProjects($mysqli, $park);
//}
...
echo <<<_END
		<script  type="text/javascript">
			...
			function onLoadFunction()
			{
				...
				setSelectField('idPark','$park')
				setSelectField('idProjectNum','$projectNum')
				setSelectField('idYearPicked','$yearPicked')
			...
			<div id='manAssets3'>
				<form method='post' id='myform' action='$_SERVER[SCRIPT_NAME]'>
					<table class='manAssTop'>
						...
						<tr>
						<td><select id='idRegion' name='region' style='width: 20em;' onChange=this.form.submit()>
							<option value=''></option>
";
...
echo"						</select></td>
							<td><select id='idPark' name='park' style='width: 6em;' onChange=this.form.submit()>
								<option value=''> </option>
";
foreach($resParks as $row) {
//while($row = $resParks->fetch_row())
	echo"								<option value='{$row['park_id']}'> {$row['park_id']} </option>
";
}
echo"							</select></td>
							<td><select id='idProjectNum' name='projectNum' style='width: 20em;' onChange=this.form.submit()>
...
echo"							</select></td>
							<td><select id='idYearPicked' name='yearPicked' style='width: 6em;' onChange=this.form.submit()>
								<option value=''> </option>
";
...
";
	if(1 == $level) echo "			<div id=yearBox>
				<table class='projectScore'>
					...
					<tbody>
						<tr>
							<td>Post results<td><td><select id='idYearSet' name='yearSet' style='width: 6em;'>
								<option value=''></option>
